        ...
        for i in range(self.num_of_agents):  # Find path for each agent
            ...
            for k in range(i+1, self.num_of_agents):
                for j in range(len(path)-1):
                    loc1, loc2 = path[j], path[j+1]
                    constraints.append({'agent': k, 'loc': [loc1], 'timestep': j})
                    constraints.append({'agent': k, 'loc': [loc2, loc1], 'timestep': j+1})
                for j in range(-1, 2*self.total_false):
                    constraints.append({'agent': k, 'loc': [loc2], 'timestep': len(path)+j})
